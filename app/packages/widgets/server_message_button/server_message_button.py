...
class ServerButton(QWidget):
    ...
    def __init__(
        ...
        self.setObjectName(str(server_id))
        self.setup_ui()
        ...
        if self.server_status == "online":
            self._status_color = "#46b946"
        elif self.server_status == "ilde":
            self._status_color = "#ff9955"
        elif self.server_status == "busy":
            self._status_color = "#a02c2c"
        elif self.server_status == "invisible":
            self._status_color = "#808080"

        # CHANGE OPACITY
        if self.server_status == "invisible":
    ...
    def change_status_color(self, status):
        if status == "online":
            self._status_color = "#46b946"
        elif status == "ilde":
            self._status_color = "#ff9955"
    ...
    def setup_ui(self):
        ...
        self.label_user.setText(self.server_name.capitalize())
        ...
        self.label_description.setText(self.server_description)
    ...
    def paintEvent(self, event):
        ...
        # DRAW USER IMAGE
        self.draw_server_image(painter, self.server_image, rect)

        painter.end()

        # DRAW USER IMAGE
        if self.server_status != "invisible":
            self.draw_status(self.server_image, rect)

    # DRAW USER IMAGE
    # ///////////////////////////////////////////////////////////////
    def draw_server_image(self, qp, image, rect):
        server_image = QImage(image)
        path = QPainterPath()
        path.addEllipse(rect)
        qp.setClipPath(path)
        qp.drawImage(rect, server_image)
